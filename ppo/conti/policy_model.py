...
class PolicyModel(object):
	...
	# Constructor
	#---------------------------
	def __init__(self, sess, logstd_ph, s_dim, a_dim, a_low, a_high, name="policy", reuse=False):
		self.sess = sess
		self.logstd_ph = logstd_ph
		...
		#neg_logprob: (mb_size)
		self.value = value[:, 0]
		self.distrib = distribs.DiagGaussianDistrib(mean, logstd_ph)
